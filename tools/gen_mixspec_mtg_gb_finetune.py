...
def gen_session(corpus_spkrwise, tgtdir, config):
    ...
    for spkr in corpus_spkrwise:
        ...
        dyn_corpus[spkr] = utterances #utterances
    ...
    while len(dyn_corpus) > 0:
        ...
        while start_spkr < len(speakers):
            ...
            for spkr in cur_spkrs:
                ...
                if len(rem) == 0:
                    dyn_corpus.pop(spkr)   #用完就把这个人的语音从语料中踢掉
                else:
                    dyn_corpus[spkr] = rem

                for i, utt in enumerate(pop):
                    utterances_in_session[i].append(utt) #utterances_in_session[i] 包含多个不同人的语音
...
def give_timing(sess, overlap_time_ratio=0.3, sil_prob=0.2, sil_dur=[0.3, 2.0], allow_3fold_overlap=False):
    time_marked_sess = copy.deepcopy(sess) #这个仿真session中的所有语料列表，json元素为单位
    ...
    to_overlap = bernoulli.rvs(1 - sil_prob, size=nutts - 1).astype(bool).tolist() # 0,1分布列表
    ...
    #     p = random.betavariate(1, 5) #以 beta 分布的概率分布返回 0 到 1 之间的随机数，1,5是0到1递减的分布
    #     probs.append(rem * p)
    #     rem *= (1 - p)
    # probs.append(rem)
    # random.shuffle(probs)   #probs和为1

    idx = -1
    overlap_times = [0.0]
    E = total_overlap_time / noverlaps
    for b in to_overlap:
        if b:
            idx += 1
            # overlap_times.append(probs[idx] * total_overlap_time)
            #hash code
            overlap_times.append(np.random.uniform(E-1, E+1))
        else:
            overlap_times.append(-np.random.uniform(low=sil_dur[0], high=sil_dur[1]))
    
    #overlap_times这个生成的很寄吧不合理，导致有些overlap很长，导致min(ot, offset - last_utt_end[spkr])取最小
    ...
    for utt, ot in zip(time_marked_sess, overlap_times):
        ...
        if len(last_utt_end_times) > 1 and (not allow_3fold_overlap): 
            ...
            ot = min(ot, offset - last_utt_end[spkr]) #不能和自己重叠
...
#     {
#     "utterance_id": "Bert-180-180",
#     "path": "/train33/sppro/permanent/gbyang/code/NSD-MS2S/local_gb/solo_wav_4s/Bert/Bert-180-180.wav",
#     "speaker_id": "Bert",
#     "number_of_samples": 66560,
#     "sampling_rate": 16000,
#     "length_in_seconds": 4.16
#   }
    ...
    sess_list = []
    for i in range(args.repeate_times):
        sess_list += gen_session(corpus_spkrwise, args.targetdir, config)
...
def make_argparse():
    ...
    parser.add_argument('--repeate_times', metavar='N', type=int, default=1,
                        help='Seed for random number generators. The current system time is used when this option is not used.')
