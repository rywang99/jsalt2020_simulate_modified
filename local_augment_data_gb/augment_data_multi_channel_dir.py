...
def augment_wav(utt_wav, dur, fg_snr_opts, bg_snr_opts, fg_noise_utts, \
...
def get_new_id(utt, utt_modifier_type, utt_modifier):
    ...
    if utt_modifier_type == "suffix" and len(utt_modifier) > 0:
        new_utt = utt + "_" + utt_modifier
    elif utt_modifier_type == "prefix" and len(utt_modifier) > 0:
        new_utt = utt_modifier + "_" + utt
    else:
        new_utt = utt
...
def main():
    ...
    for line in wav_scp_file:
        ...
        utt = toks[0]
        real_session = "_".join(utt.split("_")[:-1])
        if real_session not in session2wavs.keys():
            session2wavs[real_session] = []
        wav = " ".join(toks[1:])
        session2wavs[real_session].append([utt, wav])
    for session in session2wavs:
        utt_wav = session2wavs[session]
        dur = reco2dur[utt]
        new_utt_wav = augment_wav(utt_wav, dur, fg_snrs, bg_snrs, fg_noise_utts,
            bg_noise_utts, noise_wavs, noise_reco2dur, args.fg_interval,
            num_bg_noises)

        for utt_new_wav in new_utt_wav:
            utt, new_wav = utt_new_wav
