...
data=$(basename $noise_data)

mkdir -p data/$data

find $noise_data -type f -name *.gt_noise_*.wav | awk -F "[/]" '{split($(NF), a, ".wav"); print a[1],$0}' > data/$data/wav.scp
...
python local/gen_mixspec_rttm_ctm.py $noise_data/mixspec.json

cat $noise_data/rttm/* | awk '{$2="SimuLibri_DEV_reverb_"$2;print $0}' > data/${data}_Mixer6Noise/oracle.rttm
