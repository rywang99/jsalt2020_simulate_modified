...
if [ ! -v roomcfg ]; then
    ...
    # roomcfg=$ROOTDIR/configs/common/meeting_reverb_chime7.json
    roomcfg=$ROOTDIR/configs/common/meeting_reverb_linear_chime7.json
fi
if [ ! -v dyncfg ]; then
    dyncfg=$ROOTDIR/configs/common/meeting_dynamics_chime7_4spk.json
fi
...
#datajson=/train8/sppro/mkhe/jsalt2020_simulate/simulate_data/data/Simu_LibriSpeech_train_4spk_clean/train.json
datajson=./librispeech_clean.json
# Generate mixture specs. 
tgtdir=$tgtroot/wav
mkdir -p $tgtdir

specjson=$tgtroot/mixspec.json
python $mixspec --inputfile $datajson --outputfile $specjson --targetdir $tgtdir --random_seed 30 --config $dyncfg
