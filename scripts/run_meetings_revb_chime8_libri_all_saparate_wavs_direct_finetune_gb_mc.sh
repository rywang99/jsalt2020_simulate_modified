...
# Import $EXPROOT. 
ROOTDIR=/train33/sppro/permanent/stniu/jsalt2020_simulate
ROOTDIR=/train33/sppro/permanent/stniu/jsalt2020_simulate
...
list2json=$ROOTDIR/tools/list2json_dihard_split_SS.py
mixspec=$ROOTDIR/tools/gen_mixspec_mtg_gb_finetune.py
...
tgtroot=/train33/sppro/permanent/stniu/simulate_finetune/simulate_data/data/$1

if [ ! -v roomcfg ]; then
    # roomcfg=$ROOTDIR/configs/common/meeting_reverb_mono_M2MeT.json
    roomcfg=$ROOTDIR/configs/common/meeting_clean_chime.json
fi
if [ ! -v dyncfg ]; then
    dyncfg=$ROOTDIR/configs/common/meeting_dynamics_chime8_8spk.json
fi
echo $roomcfg $dyncfg
...
# datajson=/train33/sppro/permanent/gbyang/code/jsalt2020_simulate/simulate_data/librispeech_clean_all_suf2/all.json
...
for ch in `seq 0 6`; do
    if [ $ch -eq 0 ];
      then
        mixer=$ROOTDIR/tools/mixaudio_mtg_return_saparate_wavs.py
    else
        mixer=$ROOTDIR/tools/mixaudio_mtg.py
    fi
    
    specjson=$tgtroot/mixspec.json
    python $mixspec --inputfile $datajson --outputfile $specjson --targetdir $tgtdir --random_seed 50 --config $dyncfg --repeate_times 50
    # exit
    sed "s/ch0/ch${ch}/g" $specjson > $tgtroot/mixspec_ch${ch}.json
    specjson=$tgtroot/mixspec_ch${ch}.json
    ...
    ${gen_cmd} JOB=1:${nj} ${splitdir}/log/mixlog.JOB.log \
        python $mixer $opts --iolist ${splitdir}/mixspec_ch${ch}.JOB.json --cancel_dcoffset --random_seed JOB --sample_rate 16000 --log ${splitdir}/mixlog.JOB.json --mixers_configfile $roomcfg
    python $mergejson $(for j in $(seq ${nj}); do echo ${splitdir}/mixlog.${j}.json; done) > $mixlog
done 
mkdir -p $tgtdir/wav_all
for file in `ls $tgtdir | grep -v all | grep -v source | awk -F '.' '{print $1}'`; do 
    for ch in `seq 0 6`; do
        newdir=`echo $tgtdir | sed "s/ch0/ch${ch}/g"`
        # echo $newdir
        ln -s $newdir/${file}.wav $tgtdir/wav_all/${file}_ch${ch}.wav
